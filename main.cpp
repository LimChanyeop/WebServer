		...
		for (int i = 0; i < num_of_event; i++)
		{
			...
			else if (kq.event_list[i].filter == EVFILT_READ)
			{
				...
				else if (clients.find(id) != clients.end()) // 이벤트 주체가 client
				{
					...
					std::cerr << "GO!::::" << clients[id].request.start_line << std::endl;
					...
					if (clients[id].request.get_method() == "GET")
					{
						...
						else if (location_id == -2) // is file
						{
								...
								clients[id].get_route().find(".py") != std::string::npos)
							{
								...
								fcntl(clients[id].read_fd, F_SETFL, O_NONBLOCK);
					...
					////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					else if (clients[id].request.get_method() == "POST" && clients[id].get_status() != WAIT) /////////////// *************** post ***************** ////////////////////////////////////////////////////////////
					{
						...
						if (is_dir == 1) // is dir
						{
							...
							std::cerr << "referer: " << clients[id].request.get_referer() << std::endl;
							std::cerr << "post_filename: " << clients[id].request.post_filename << std::endl;
							std::cerr << "post_content_type: " << clients[id].request.post_content_type << std::endl;
						...
						else ///////////////////////////////////// POST
						{
								...
								clients[id].request.get_referer().find("py") != std::string::npos)
							{
								...
								webserv.run_cgi(Config.v_server[server_id], index_root, clients[id]);
								clients[clients[id].read_fd].pid = clients[id].pid;
								clients[clients[id].write_fd].pid = clients[id].pid;
								clients[clients[id].read_fd].set_read_fd(id);
								clients[clients[id].write_fd].set_write_fd(id);
								clients[clients[id].read_fd].request.post_body = clients[id].request.post_body;
								clients[clients[id].write_fd].request.post_body = clients[id].request.post_body;
								clients[clients[id].write_fd].set_status(need_to_cgi_write);

								std::cerr << "clients[read_fd:" << clients[id].read_fd << "].set_read_fd(" << id << ")\n";
								std::cerr << "clients[write_fd:" << clients[id].write_fd << "].set_write_fd(" << id << ")\n";
								std::cout << "read_fd : " << clients[id].read_fd << ", write_fd : " << clients[id].write_fd <<std::endl;

								clients[id].set_status(WAIT);
								fcntl(clients[id].write_fd, F_SETFL, O_NONBLOCK);
								change_events(kq.change_list, clients[id].write_fd, EVFILT_WRITE, EV_ADD | EV_ENABLE, 0, 0, NULL); // cgi에 post_body 쓰기
								break;
							}
							int open_fd;
			...
			else if (kq.event_list[i].filter == EVFILT_WRITE)
			{
				...
				if (clients[id].get_status() == need_to_POST_write) ////////////////////////////////
				{
					...
					fwrite(clients[id].request.post_body.c_str(), sizeof(char), clients[id].request.post_body.length(), fp);
				...
				{
					std::cerr << "im POST-CGI write!! id: " << id << ", and write to " << clients[id].write_fd << ".\n"; 
					FILE *fp = fdopen(id, "wb");

					fwrite(clients[id].request.post_body.c_str(), sizeof(char), clients[id].request.post_body.length(), fp);
					// write(id, clients[id].request.post_body.c_str(), clients[id].request.post_body.length());
					std::cerr << "write-" << id << ":" << clients[id].request.post_body << std::endl;
					int read_fd = clients[clients[id].write_fd].read_fd;
					clients[read_fd].set_status(need_to_cgi_read);
					change_events(kq.change_list, read_fd, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL); // cgi result 읽기
				...
				else if (clients[id].get_status() >= WRITE_LINE)
				{
					...
					if (clients[id].get_status() >= WRITE_LINE) // && server_it->request.get_host() != "")
					{
						...
						if (clients[id].get_status() == cgi_read_ok)
						{
							...
							fwrite(clients[id].response.get_send_to_response().c_str(), sizeof(char), clients[id].response.get_send_to_response().length(), fp);
							std::cerr << "END::::" << clients[id].request.start_line << std::endl;
							close(id);
							fclose(fp);
							clients.erase(id);
							break;
