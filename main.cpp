        ...
        for (int i = 0; i < num_of_event; i++) {
            ...
            if (kq.event_list[i].filter == EVFILT_READ) {
                ...
                if (find_server(Config, kq.event_list[i].ident, server_it)) // 이벤트 주체가 server
                {
                    ...
                    int acc_fd;
                    // std::cout << "BEFORE errno: " << errno << std::endl;
                    std::cout << "BEFORE strerror: " << strerror(errno) << std::endl;
                    std::cout << server_it->get_socket_fd() << std::endl;
                ...
                } else if (kq.clients.find(kq.event_list[i].ident) != kq.clients.end()) // 이벤트 주체가 client
                {
                    ...
                    int valread;
                    std::string request;
                    if (kq.event_list[i].ident == 0) {
                        valread = read(kq.event_list[i].ident, READ, 1023);
                        std::cout << "ident=0: " << READ << std::endl;
                    }
                    std::cout << "1strerror: " << strerror(errno) << std::endl;
                    ...
                    if (server_it->request.get_referer().find("php") != std::string::npos) /////////////////// cgi
                    {
                        ...
                        pipe(write_fd);
                        std::cout << "2strerror: " << strerror(errno) << std::endl;
                        std::cout << "ar[0]" << server_it->get_cgi_path().c_str() << std::endl;
                        std::cout << "ar[1]"
                                  << Config.v_server[server_id].v_location[location_id].get_root() + "/" +
                                         Config.v_server[server_id].v_location[location_id].get_index()
                                  << std::endl;
                        int pid = fork();
                        if (pid == -1) {
                            std::cerr << "fork error\n";
                        ...
                        } else if (pid == 0) // child
                        {
                            ...
                            ar[1] =
                                strdup((Config.v_server[server_id].get_root() + "/" + Config.v_server[server_id].v_location[location_id].get_index())
                                           .c_str()); // file name(./file)
                            ar[2] = 0;
                        ...
                        std::cout << "3strerror: " << strerror(errno) << std::endl;
                        close(write_fd[0]);
                        close(read_fd[1]);
                        std::cout << "4strerror: " << strerror(errno) << std::endl;
                        int ret;
                        ...
                        std::cout << "5strerror: " << strerror(errno) << std::endl;
                    } else {
                        ...
                        std::string route =
                            Config.v_server[server_id].get_root() + '/' + Config.v_server[server_id].v_location[location_id].get_index();
                ...
                std::cout << "6strerror: " << strerror(errno) << std::endl;
            } // if /READ
                     ...
                     kq.clients[kq.event_list[i].ident].get_server_sock() == server_it->get_socket_fd()) // && server_it->request.get_host() != "")
            {
                ...
                if (kq.clients[kq.event_list[i].ident].get_status() == ok) // && server_it->request.get_host() != "")
                {
                    ...
                    std::cout << "7strerror: " << strerror(errno) << std::endl;
