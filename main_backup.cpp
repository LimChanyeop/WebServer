	...
	while (1) {
		...
		for (int i = 0; i < num_of_event; i++)
		{
			...
			if (kq.event_list[i].filter == EVFILT_READ)
			{
				...
				// if (kq.clients[kq.event_list[i].ident].get_read_fd() != -1)
				// {
				// 	read(kq.event_list[i].ident)
				// }
				// if (kq.clients[kq.event_list[i].ident].get_read_fd() != -1 && kq.clients[kq.event_list[i].ident].get_read_fd() == kq.event_list[i].ident)
				// {
				// 	// file Read
				// 	FILE *resource_ptr = fdopen(kq.event_list[i].ident, "r");
				...
				// 	{
				// 		// client_socket->setErrorResource("500"); // 500
				// 		continue;
				// 	}
				// 	server_it->response.response_lenth = resource_size;
				// 	server_it->response.response_str.append(buf2, resource_size);
				// 	// server_it->setStage(MAKE_RESPONSE);
				// 	fclose(resource_ptr);
				...
				else if (kq.clients.find(kq.event_list[i].ident) != kq.clients.end()) // 이벤트 주체가 client
				{
					...
					else
					{
						...
						kq.clients[kq.event_list[i].ident].set_route(Config.v_server[server_id].get_root() + '/' + Config.v_server[server_id].v_location[location_id].get_index());
						//	/Users/minsikkim/Desktop/WeL0ve42Seoul/WebServer/View + / + Default.html
						// std::cout << "route: "<<route << std::endl;

						std::ifstream ifs(kq.clients[kq.event_list[i].ident].get_route().c_str());
				...
				kq.clients[kq.event_list[i].ident].get_server_sock() == server_it->get_socket_fd())// && server_it->request.get_host() != "")
			{
				...
				{
					if (server_it->request.referer.find("favicon.ico") != std::string::npos && server_it->request.get_method() == "GET")
						server_it->response.set_response(42, server_it->response.get_response(), ""); // 42
					else if (server_it->request.referer.find("favicon.ico") == std::string::npos && server_it->request.get_method() == "GET")
						server_it->response.set_response(200, server_it->response.get_response(), ""); // OK
					else if (server_it->request.get_method() == "POST" && atoi(server_it->request.get_contentLength().c_str()) == 0) // POST
						server_it->response.set_response(411, server_it->response.get_response(), kq.clients[kq.event_list[i].ident].get_route());
					else if (server_it->request.get_method() == "POST") // POST 201 Created
						server_it->response.set_response(201, server_it->response.get_response(), kq.clients[kq.event_list[i].ident].get_route());
					else
						server_it->response.set_response(404, server_it->response.get_response(), "");
					...
					server_it->response.set_response(0, "", "");
					std::cout << "response..\n\n";
				}
			} // else if /WRITE
		}
