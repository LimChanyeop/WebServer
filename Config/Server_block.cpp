...
Server_block::Server_block(Base_block bb)
{
	bb.set_client_limit_body_size(this->get_client_limit_body_size());
	bb.set_request_limit_header_size(this->get_request_limit_header_size());
	bb.set_user(this->get_user());
	bb.set_worker_processes(this->get_worker_processes());
	bb.set_root(this->get_root());
	bb.set_index(this->get_index());
	bb.set_autoindex(this->get_autoindex());
	bb.set_return_n(this->get_return_n());
	bb.set_error_page(this->get_error_page());
	bb.set_auth_key(this->get_auth_key());
	bb.set_cgi_info(this->get_cgi_info());
	bb.set_allow_methods(this->get_allow_methods());
}
...
void Server_block::set_user(std::string str)
{
	user = str;
}
...
void Server_block::config_parsing(std::vector<std::string>::iterator &it, std::vector<std::string> &lists) //, Config_base config_base)
{
	for (; it != lists.end() && *it != "}"; it++)
	{
		...
		case 14: // server
			...
			break;
		case 15:														 // location
			locations.push_back(Location_block(*this));					 // vec locations push_back
			locations[locations.size() - 1].config_parsing(++it, lists); // locations[i].server_block_parsing((&)it, conf_lists)
			break;
